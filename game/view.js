...
var view = new (function () {
    ...
    this.fillArc = function (x, y, radius, color, angle, scaleX, scaleY) {
        ...
        ctx.translate(x, y);
        ctx.rotate(angle);
        if (scaleX && scaleY) ctx.scale(scaleX, scaleY);
        ctx.beginPath();
    ...
    this.drawMissile = function (curve, t, tail, radius, color, scaleX, scaleY) {
        ...
        while (i-- > 0) {
            ...
            if (i > 0) ctx.lineTo(tail[i - 1][0], tail[i - 1][1]);
    ...
    SourceReady.ready(function () {
        terrain.dimensions.then(function (width, height) {
            canvasContainer.style.width =
                (canvasFoW.width = canvas.width = canvasTerrain.width = viewWidth = width) + "px";
            canvasContainer.style.height =
                (canvasFoW.height = canvas.height = canvasTerrain.height = viewHeight = height) + "px";
