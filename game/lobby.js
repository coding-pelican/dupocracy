        ...
        gc.getConnection("dupocracy").then(function (connection) {
            ...
            connection.hon("roomPong", function (header, body, data, clientID) {
                rooms.some(function (room) {
                    if (room.name == body) {
                        ...
                        } else room.pingMap[clientID] = Date.now();
            ...
            connection.on("roomPingMap", function (header, body) {
                ...
                rooms.every(function (room) {
                    if (room.name == roomName) room.pingMap = pingMap;
                    else return true;
                });
            });
            ...
            setInterval(function () {
                rooms.forEach(function (room) {
                    if (Date.now() - room.lastPing > 30 * 1000) connection.broadcast("removeRoom", room.name);
                    else var deleted = false;
                    ...
                    if (deleted) connection.broadcast("roomPingMap", room.name + ":" + JSON.stringify(room.pingMap));
                });
