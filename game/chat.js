                ...
                function (header, mySelf) {
                    ...
                    connection.on("clients", function (header, body) {
                        ...
                        connection.on("chatMsg", function (header, body) {
                            ...
                            var sender = clients.filter(function (client) {
                                return client.clientID == msg.clientID;
                            })[0].name;
                        ...
                        connection.hon("chatLog", function (header, body, data, clientID) {
                            if (chatLog.length == 0) addLine(mySelf + " creates room");
                            else connection.toClient(clientID, "chatLog", JSON.stringify(chatLog.slice(0, 100)));
                        });
                        ...
                        connection.hon("leave", function (header, body) {
                            ...
                            clients = clients.filter(function (client) {
                                if (client.clientID == clientID) addLine(client.name + " leaves game");
                                else return true;
                            });
                        });
                    });
