var UI = new (function () {
    ...
    this.FactionWidget = function (factions) {
        ...
        this.clearAll = function (slot) {
            factions.some(function (slot) {
                if (factionNode[slot].querySelector("button")) clearSlot(slot);
            });
        };
    ...
    this.nameDialog = function () {
        ...
        promptDialogNode.innerHTML =
            "<h3>Enter your name</h3><center>" +
            '<input type="text" name="name" placeholder="Type your name here" />' +
            "<button>Join</button>" +
            '<p class="error"> </p></center>';

        promptDialogNode.querySelector("button").addEventListener(
            "click",
            function () {
    ...
    this.roomLobby = function (rooms) {
        ...
        var addRoom = (result.addRoom = function (room) {
            ...
            node.innerHTML =
                ...
                " <i>#players: " +
                Object.keys(room.pingMap).length +
                "</i>" +
                "</label>" +
                "<button>Join</button>";
    ...
    this.contextMenu = function (x, y, options) {
        if (currentMenu) UI.remove(currentMenu);
    ...
    this.toggleNode = function (node, enabled) {
        if (enabled && !node.parentNode) document.body.appendChild(node);
        else if (!enabled && node.parentNode) remove(node);
    };
    ...
    this.hideStatus = function () {
        if (statusEl) statusEl = remove(statusEl);
    };
    ...
    this.updateWorldTime = function (t) {
        if (!(t > 0)) return this.toggleNode(wtEl, false);
        else this.toggleNode(wtEl, true);
    ...
    function escapeHTML(str) {
        if (str) return str.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
    }
})();
