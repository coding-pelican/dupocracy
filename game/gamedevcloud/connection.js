(function (global) {
    function Connection(socket, server) {
        ...
        this.on(
            ...
            function (header, body) {
                if (header == "host") this.isHost = true;
                if (header == "client") this.clientID = body;
            }.bind(this)
        );

        this.hon(
            "ping",
            function (header, body, data, clientID) {
        ...
        if (this.socketURL == "loopback") this.loopback();
        else
            try {
                this.socket = socket;

                this.socket.onerror = this.socket.onclose = this.closeSocket.bind(this);
    ...
    Connection.prototype.broadcast = function (header, body, callback) {
        if (this.isHost) {
            if (this.socket) this.socket.send("*:" + header + ":" + (body != null ? body : ""));
            setTimeout(
                function () {
    ...
    Connection.prototype.toHost = function (header, body, callback) {
        if (this.isHost && header != "ping")
            setTimeout(
                function () {
                    this.receive(this.clientID, header + ":" + (body != null ? body : ""));
                }.bind(this),
                0
            );
        else if (this.socket) this.socket.send("host:" + header + ":" + (body != null ? body : ""));
    };
    ...
    Connection.prototype.toClient = function (clientID, header, body, callback) {
        if (this.isHost)
            ...
            else if (this.socket) this.socket.send(clientID + ":" + header + ":" + (body != null ? body : ""));
    };
