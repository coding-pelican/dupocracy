(function (global) {
    ...
    StateMachine.prototype.feed = function (currentState, eventType, targetState, handler) {
        this.table.push({
    ...
    StateMachine.prototype.event = function (eventType) {
        this.table.every(
            function (feed) {
                if (
                    (((this.bitModeStates && (feed.currentState & this.currentState) > 0) ||
                        (!this.bitModeStates && feed.currentState === this.currentState)) &&
                        this.bitModeEvents &&
                        (feed.eventType & eventType) > 0) ||
                    (!this.bitModeEvents && feed.eventType === eventType)
                ) {
                    if (feed.handler) feed.handler(eventType);
