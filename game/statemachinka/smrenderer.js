(function (global) {
    function SMRenderer(sm) {
        ...
        sm.table.reduce(function (l, feed) {
            if (bitMode.bitStates) stateMask = stateMask | feed.currentState | feed.targetState;
            else {
                if (states.indexOf(feed.currentState) == -1) states.push(feed.currentState);
                if (states.indexOf(feed.targetState) == -1) states.push(feed.targetState);
            }

            if (bitMode.bitEvents) eventMask = eventMask | feed.eventType;
            else if (events.indexOf(feed.eventType) == -1) events.push(feed.eventType);
        });

        if (bitMode.bitStates) for (var i = 0; i < 32; i++) if ((stateMask >> i) & 1) states.push(1 << i);

        if (bitMode.bitEvents) for (var i = 0; i < 32; i++) if ((eventMask >> i) & 1) events.push(1 << i);
        ...
        sm.event = function (eventType) {
            ...
            if (currentState != this.currentState) {
                if (!stateMap[currentState]) return;
                ...
                ctx.quadraticCurveTo(
                    (stateMap[currentState][0] + stateMap[this.currentState][0]) / 2,
                    (stateMap[currentState][1] + stateMap[this.currentState][1]) / 2,
                    stateMap[this.currentState][0] + 7,
                    stateMap[this.currentState][1] + 7
                );
                ctx.stroke();
            }
        };
