var world = new (function () {
    ...
    var visibility = population.reduce(function (r, city) {
        if (!r[city.faction]) r[city.faction] = { V: {}, H: {} };
        return r;
    }, {});
    ...
    function update() {
        ...
        while (afterListeners[0] && afterListeners[0].t <= worldTime) afterListeners.splice(0, 1)[0].fn(worldTime);
    ...
    function updateExplosions(dt) {
        ...
        while (i-- > 0) {
            ...
            if (worldTime - explosion.opts.explosionTS > 3000) remove(explosion.id);
        }
    }
        ...
        while (i-- > 0) {
            ...
            if (launcher.opts.target) {
                ...
                if (launcher.opts.mode == 1 && launcher.opts.targetMode == 1)
                    add("missile", launcher.x, launcher.y, {
                        sx: launcher.x,
                        sy: launcher.y,
                        tx: launcher.opts.target[0],
                        ty: launcher.opts.target[1],
                        faction: launcher.opts.faction,
                    });
                else if (launcher.opts.mode == 2 && launcher.opts.targetMode == 2)
                    add("scout", launcher.x, launcher.y, {
                        tx: launcher.opts.target[0],
                        ty: launcher.opts.target[1],
                        faction: launcher.opts.faction,
                    });
    ...
    function updateMissiles(dt) {
        ...
        while (j-- > 0) {
            ...
            if (missile.ft < 100 && !missile.dead) {
                if (!missile.V) missile.V = VMath.normalize([missile.opts.tx - missile.x, missile.opts.ty - missile.y]);
                var V = missile.V;
                missile.opts.t =
                    VMath.distance([missile.x, missile.y], [missile.opts.tx, missile.opts.ty]) / missile.opts.dist;
                ...
                if (VMath.length([missile.opts.tx - missile.x, missile.opts.ty - missile.y]) < 8 && !missile.dead) {
                    ...
                    launchers.some(function (launcher) {
                        if (VMath.distance([missile.x, missile.y], [launcher.x, launcher.y]) < 16) remove(launcher.id);
                    });
                    radars.some(function (radar) {
                        if (VMath.distance([missile.x, missile.y], [radar.x, radar.y]) < 16) remove(radar.id);
                    });
    ...
    function fillSight(object) {
        if (!object.dead && object.visibilityRadius > 0) {
            if (!sight[object.opts.faction]) sight[object.opts.faction] = { V: {}, H: {} };
            ...
            for (var i = left; i <= right; i++) sight[object.opts.faction].H[i] = true;
            for (var i = top; i <= bottom; i++) sight[object.opts.faction].V[i] = true;
        }
    }
    ...
    function triggerInterceptors(worldTime, dt) {
        ...
        while (i-- > 0) {
            var launcher = launchers[i];
            if (
                launcher.opts.nextTickTS > worldTime ||
                launcher.opts.mode != 0 ||
            ...
            while (j-- > 0) {
                var missile = missiles[j];
                if (
                    launcher.opts.faction != missile.opts.faction &&
                    visibilityCheck(missile, sight[launcher.opts.faction], launcher.opts.faction)
                )
    ...
    function updateLauncher(launcher, worldTime, dt, missile) {
        ...
        if (
            (!target && VMath.distance([launcher.x, launcher.y], missile.opts.proj) < 500) ||
            (target &&
                launcher.opts.nextLaunchTS < worldTime &&
                VMath.distance([launcher.x, launcher.y], target.opts.proj) < 500)
        ) {
            launcher.opts.nextLaunchTS = worldTime + 2000;
            launcherTargets[launcher.id] = missile.id;
            add("interceptor", launcher.x, launcher.y, {
                sx: launcher.x,
                sy: launcher.y,
                tx: missile.x,
                ty: missile.y,
    ...
    function updateInterceptors(dt) {
        ...
        while (j-- > 0) {
            ...
            var dP =
                target && target.opts.proj
                    ? [target.opts.proj[0] - interceptor.x, target.opts.proj[1] - interceptor.y]
                    : interceptor.V;
    ...
    function updateScouts(dt) {
        ...
        while (j-- > 0) {
            ...
            if (scout.ft < 100 && !scout.dead) {
                if (!scout.V) scout.V = VMath.normalize([scout.opts.tx - scout.x, scout.opts.ty - scout.y]);
                else {
                    var V = VMath.normalize([
                        scout.opts.tx - scout.x + Math.cos(scout.ft) * 20,
    ...
    var after = (this.after = function (t, fn) {
        if (!lastUpdate) this.run();

        var event = { t: worldTime + t, fn: fn };

        if (
            !afterListeners.some(function (listener, idx) {
    ...
    this.countGroup = function (type, faction) {
        ...
        if (group)
            return group.reduce(function (count, object) {
                if (object.opts.faction == faction) return count + 1;
                else return count;
            }, 0);
        else return 0;
    };
    ...
    this.canAdd = function (type, x, y, opts) {
        ...
        if (type == "launcher" || type == "radar") {
            return (
                this.countGroup(type, opts.faction) < 5 &&
                population.some(function (hotspot) {
                    return hotspot.faction == opts.faction && VMath.distance([x, y], [hotspot.x, hotspot.y]) < 100;
                }) &&
    ...
    var add = (this.add = function (type, x, y, opts, id) {
        if (!id)
            ...
            if (object.type == "missile") {
                ...
                var S = [sx, sy],
                    E = [tx, ty];
                object.opts.curve = [S, VMath.add(S, VMath.sub(VMath.scale(VMath.sub(E, S), 0.5), N)), E];
                object.opts.t = 0;
    ...
    var remove = (this.remove = function (objectID) {
        ...
        if (IDmap[objectID]) delete IDmap[objectID];
        groups.some(function (group) {
            if (
                group.some(function (object, idx) {
    ...
    this.query = function (x, y, r, rect) {
        ...
        groups.some(function (group) {
            return group.some(function (object) {
                if (
                    ((!object.dead && rect && VMath.insideRect([object.x, object.y], rect)) ||
    ...
    var onSight = (this.onSight = function (asFaction, fn, filter) {
        ...
        (sightListeners[asFaction] || (sightListeners[asFaction] = [])).push({
            faction: asFaction,
            fn: fn,
            filter: filter || {},
            mask: {},
        ...
        for (var faction in sightListeners)
            sightListeners[faction].some(function (listener) {
                groups.some(function (group) {
                    return group.some(function (object) {
                        if (
                            object.opts.faction != faction &&
                            listener.filter[object.type] &&
                            !listener.mask[object.id] &&
    ...
    var updateVisibility = function (visibleFaction) {
        if (visibleFaction && new Date().getTime() - (visibility[visibleFaction].lastUpdate || 0) > 1000) {
            ...
            function fill(object) {
                ...
                var right = Math.floor((object.x + object.visibilityRadius) / 16);
                var bottom = Math.floor((object.y + object.visibilityRadius) / 16);
                for (var i = left; i <= right; i++) visibility[object.opts.faction].H[i] = true;
                for (var i = top; i <= bottom; i++) visibility[object.opts.faction].V[i] = true;
            }
            ...
            population.some(function (city) {
                if (city.faction == visibleFaction) fill(city);
            });
        }
    };

    this.render = function () {
        for (var faction in visibility) updateVisibility(faction);

        population.some(function (hotspot) {
            if (visibleFaction == hotspot.faction) view.lightUp(hotspot.x, hotspot.y, hotspot.visibilityRadius);
        ...
        groups.some(function (group) {
            group.some(function (object) {
                ...
                if (
                    object.dead ||
                    (!visibilityMap[object.id] &&
                        !world.visibilityCheck(object, visibility[visibleFaction], visibleFaction))
                )
                    return;

                if (!visibilityMap[object.id]) visibilityMap[object.id] = true;

                if (visibleFaction == object.opts.faction && object.visibilityRadius > 0)
                    view.lightUp(object.x, object.y, object.visibilityRadius);

                var color = object.selected
                    ? "yellow"
                    : object.opts.mode == 0
                    ? "blue"
                    : object.opts.mode == 2
                ...
                if (object.type == "launcher") {
                    ...
                    view.fillArc(
                        ...
                        object.width / 8,
                        color,
                        object.V && Math.atan2(object.V[1], object.V[0]),
                        1,
                        1
                    );
